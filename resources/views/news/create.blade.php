...
@section('content')
    ...
    <div class='form-group required' id="page-name">
        ...
        <div class="text-muted">Если название страницы openDay, то страница будет доступна по ссыке /open_day</div>
        @error('pageName')
            <div class="text-danger">Поле обязательно для заполнения</div>
        @enderror
    </div>
    <div class='form-group required' id="link-on-another-site" style="display: none">
        <label for="subject">Ссылка на страницу</label>
        {!! Form::text('link', null, ['class' => 'form form-control', 'placeholder' => 'ссылка на страницу']) !!}
        @error('link')
        <div class="text-danger">Поле обязательно для заполнения и должна быть валидна (https:\\example.com)</div>
        @enderror
    </div>
    @if ($errors->any())
